...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    if (strcmp(name, "li") == 0) {
        long int full_num;
        if (translate_num(&full_num, args[1], -2147483648, 4294967295) != 0) {
            return 0;
        }
        ...
        int upper = full_num >> 16; // 0xffff0000 -> 0x0000ffff
        int lower = full_num & 0x0000ffff; // 0xffffffff -> 0x0000ffff (not sure if this is necesary)
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = 0 | rs | rt | rd | funct;
...
int write_shift(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = 0 | rt | rd | shamt << 6 | funct;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_jr(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = 0 | rs | funct;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_itype(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = 0 | op | rs | rt | immediate;
...
int write_mem(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = 0 | op | rs | rt | offset;
...
int write_lui(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = 0 | opcode << 26 | rt | immediate;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
    ...
    int rs = translate_reg(args[0]) << 21;
    int rt = translate_reg(args[1]) << 16;
    uint16_t rel_address = ((get_addr_for_symbol(symtbl, args[2]) - addr) / 4 - 1) & 0xffff;

    uint32_t instruction = 0 | opcode << 26 | rs | rt | rel_address;
    write_inst_hex(output, instruction);
...
int write_jump(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* reltbl) {
    ...
    uint32_t instruction = 0 | opcode << 26;
    write_inst_hex(output, instruction);
    return 0;
}

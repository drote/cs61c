...
int translate_num(long int* output, const char* str, long int lower_bound, 
    ...
    *output = strtol(str, &endptr, 0); // assuming base 16 numbers are always prefixed with 0X or 0x
    valid_number = *output != 0 && *endptr == '\0';
    within_range = *output >= lower_bound && *output <= upper_bound;

    if (valid_number && within_range) {
        return 0;
    }
